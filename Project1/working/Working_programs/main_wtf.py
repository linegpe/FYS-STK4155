...
filename = "R2_alpha001.txt"
f = open(filename, "w")
#f.write("MSE train   MSE test\n")
f.write("R2 train    R2 test\n")
f.close()

for deg in range(PolyDeg):

	# Set up design matrix and scale the data
	X = DesignMatrix(x_scaled,y_scaled,deg)
	...
	#WriteToFile(filename, "MSE", z_train, z_predict, z_tilde, z_test)
	WriteToFile(filename, "R2", z_train, z_predict, z_tilde, z_test)
