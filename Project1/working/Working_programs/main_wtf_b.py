...
filename = "b_N40_alpha001.txt"
f = open(filename, "w")
f.write("MSE test   MSE train\n")
#f.write("MSE    Variance    Bias\n")
f.close()
...
if(option == "complexity"):
	...
	for i in range(MaxPolyDeg+1):
		...
		#f.write("{0} {1} {2}\n".format(MSError_test[i],Variance_test[i],Bias_test[i]))
	...
	plt.plot(ModelComplexity,MSError_test,label="Test",color="deeppink")
	plt.plot(ModelComplexity,MSError_train_boot,label="Train", color="limegreen")
	plt.xlabel("Polynomial degree",fontsize=12)
	plt.ylabel("MSE",fontsize=12)
	plt.title(r"MSE for the test and train datasets, $\alpha$=0.01, OLS",fontsize=12)
	plt.grid()
	plt.legend()
	plt.show()


	plt.plot(ModelComplexity,Variance_test,label="Variance",color="orange")
	plt.plot(ModelComplexity,Bias_test,label="Bias",color="steelblue")
	plt.plot(ModelComplexity,MSError_test,label="MSE",color="deeppink")
	plt.xlabel("Polynomial degree",fontsize=12)
	plt.ylabel("Estimated error",fontsize=12)
	plt.title(r"Bias-variance tradeoff for the test dataset, $\alpha$=0.01, OLS",fontsize=12)
	plt.grid()
	plt.legend()
	plt.show()
