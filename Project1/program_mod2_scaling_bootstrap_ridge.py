...
for lam in range(nlambdas):
	...
	MSError_train[lam] = np.mean(np.mean((z_train_new - z_predict_train)**2, axis=1, keepdims=True) )
	Bias_train[lam] = np.mean( (z_train_new - np.mean(z_predict_train, axis=1, keepdims=True))**2 )
	Variance_train[lam] = np.mean( np.var(z_predict_train, axis=1, keepdims=True) )
