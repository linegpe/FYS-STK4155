...
for i in range(MaxPolyDeg+1):
	...
	for j in range(NumBootstraps):
		...
		lasso_fit = skl.Lasso(alpha=0.01, max_iter=10e5, tol=0.01, normalize=True, fit_intercept=False).fit(X_,z_)
...
plt.plot(ModelComplexity,MSError_train_boot,label="Train MSE")
